...
ivec = oct29;%oct10_donor_nowait;%[22606:22655,22707:22755,22756: 22805]; %[jul31_donor_ran,jul31_uf1_ran,jul31_uf2_ran];
acq_time = 10;
%%%
show_fit=1;
show_images=1;
...
if isempty(c1index)
    ...
    ave_FRET = [ave_FRET, ave_FRET1];
    start_nums = [start_nums, start_nums(end)+50];
    end_nums = [end_nums, end_nums(end)+50];
    spacer = 15;  
...
sumxmat = sum(xmat,2);
sumxmat = [sumxmat; (sumxmat(start_nums(1):end_nums(1)) + sumxmat(start_nums(2):end_nums(2)))/2];
F(ind) = struct('cdata',[],'colormap',[]);
fig1 = figure(2);
for j = 1:length(ave_FRET)
    clf;
...
%movie2avi(F,'C:\Users\Bryan\Documents\MATLAB\movies\oct29_30minc_noran_averaged','compression','none');
toc