...
ngr = 16;%jind*100000;
split_matin = 16; %Set to 1 to "split" set into one group, set to >1 for number of matins you want
...
w1step = .01; w1min= .2; w1max = .2;%1.73    %1.064-2016/2/27  --- %1.73  %1.6 used for extract 8/27 and 9/5 (actual 9/5 is 1.59) %1.5 used for taxol extract; %.8-2 1.05 %w1min must be an integer multiple of w1step.
w2step = .01; w2min =  3.77; w2max = 3.77; %3.67  %3.802     %3.82/1.49   %3.8 used for 3/7/15 data %3.745 usd for 8/27 E %3.87 used for taxol extract; %3.81 was found for 8/25 b80 and 8/24 extract
...
for dataname_ind = dataname_cell
    ...
    for expt = 1:exptmax %determined by number of time series
        ...
        for cindex = 1:cyclesmax %determined by number of w2 spots
            ...
            cpath = 'C:\Users\Bryan\Documents\MATLAB\data\2016-06-23\';
            ...
            irfname = 'irf';
            data_shift_name = 'UF4_2e5_100sec_noasters';%'tholand1_128x128_sec3'; %The IRF can be a little offset (in time) from the data, this data is used to align/find the offset and shift the data
            wigsname = 'wigs';
            ...
            while jind < jmax %jind is pixel group# jmax is the total number of pixel groups
                    ...
                    for k = 1: combine_exposures_FLIMage
                        ...
                        if k==1
                            [combined_FLIMage,jmax,combined_ni] = ...
                                spc_2_his(tmini,tmaxi,dataname2,pth_data,...
                                ngr,tsm,'FLIMage',cpath,'atto565',reach);
                        else
                            [pmat,jmax,ni] = spc_2_his(tmini,tmaxi,...
                                dataname2,pth_data,ngr,tsm,'FLIMage',cpath,'atto565',reach);
                ...
                elseif jind == 1 && num_int_bins==0
                    [pmat,jmax,ni] = spc_2_his(tmini,tmaxi,dataname,...
                        pth_data,ngr,tsm,pth_irf,'atto565_2e5_pdms_100sec');
                elseif jind == 1 && num_int_bins>0
                    [pmat,~,ni] = spc_2_his(tmini,tmaxi,dataname,...
                        pth_data,ngr,tsm,'FLIMage',pth_irf,'atto565_2e5_pdms_100sec');
                end
                p = pmat(jind,:);
                    ...
                    for k = 1:num_int_bins
                        ...
                        for l = 1:length(ni)
                            if intbin(k) <= ni(l) && intbin(k+1)> ni(l)
                                ...
                                nph = ni(l)+nph;
                                datag = pmat(l,:)+datag;
                            elseif l == length(ni) && intbin(k) <= ni(l) && intbin(k+1)> ni(l)
                                count = count  +1;
                                nph = ni(l)+nph;
                                datag = pmat(l,:)+datag;
                            end
    ...
    if split_matin <2
        ...
        save(MatName, 'input');
        if ~isempty(w1vec)
            set_matnum = change_lifetimes(MatName,w1vec,set_matnum);
        end
    else
        input_holdon = input;
        for k = 1:split_matin
            ...
            [MatName,SimName] = write_to_mlist(set_matnum);
            if k==1 % This section prints out the matin #s and calculates the final matin
