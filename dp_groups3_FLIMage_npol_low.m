...
num_int_bins = 0;
combine_exposures_FLIMage =1; %Use for
reach = 2;% Used for boxcar averaging FLIM data
ngr = 1;%jind*100000;
...
w1step = .01; w1min= 1.47; w1max = 1.47;%1.73    %1.064-2016/2/27  --- %1.73  %1.6 used for extract 8/27 and 9/5 (actual 9/5 is 1.59) %1.5 used for taxol extract; %.8-2 1.05 %w1min must be an integer multiple of w1step.
w2step = .01; w2min =  3.7; w2max = 3.7; %3.67  %3.802     %3.82/1.49   %3.8 used for 3/7/15 data %3.745 usd for 8/27 E %3.87 used for taxol extract; %3.81 was found for 8/25 b80 and 8/24 extract
...
thr = 0.01;
dataname_cell = {'e1-m5-s1','e1-m5-s2','e1-m5-s3','e1-m5-s4','e1-m5-s5','e1-m5-s6'};
for dataname_ind = dataname_cell
    for reach = 2:2
        fprintf('\n\n dataname is %s',dataname_ind{1});
        ...
        for expt = 1:exptmax %determined by number of time series
            ...
            for cindex = 1:cyclesmax %determined by number of w2 spots
                ...
                cpath = 'C:\Users\Bryan\Documents\MATLAB\data\2015-2-25\';
                pth_irf = cpath; %file path IRF
                pth_data = cpath; %file path data
                pth_wigs = 'C:\Users\Bryan\Documents\MATLAB\data\2015-2-25\'; %file path wiggles
                pth_ext = cpath; %ignore this
                pth_data_for_shift = cpath;
                irfname = 'irf-430pm';
                data_shift_name = 'atto565-10um';%'tholand1_128x128_sec3'; %The IRF can be a little offset (in time) from the data, this data is used to align/find the offset and shift the data
                wigsname = 'wigs';
                extname = wigsname; %IGRNORE THIS
                
                shiftstep = .2; shiftmin = -30; shiftmax = 5;
                w2step_shift = .01; w2min_shift = 3; w2max_shift = 4;
                    ...
                    if jind ==1 && combine_exposures_FLIMage > 0
                        for k = 1: combine_exposures_FLIMage
                            if k==1
                                ...
                                %Change this so you pass a vector of the
                                %cX's you want appended
                            end
                            
                            if k==1
                                [combined_FLIMage,jmax,combined_ni] = spc_2_his...
                                    (tmini,tmaxi,dataname2,pth_data,ngr,tsm,...
                                    'FLIMage',cpath,'atto565-10um',reach);
                            else
                                [pmat,jmax,ni] = spc_2_his(tmini,tmaxi,...
                                    dataname2,pth_data,ngr,tsm,'FLIMage',...
                                    cpath,'atto565-10um',reach);
