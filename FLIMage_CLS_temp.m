...
%%
figure(1); clf; imagesc(flimap); axis square;
set(gca,'XTickLabel','','XTick',[],'YTickLabel','','YTick',[]);
hcb=colorbar('eastoutside');
set(hcb,'YTick',sample{5},'YTickLabel',[]); drawnow;

figure(2); clf; imagesc(intensity); axis square;
set(gca,'XTickLabel','','XTick',[],'YTickLabel','','YTick',[]);
gcb = colorbar('westoutside');
set(gcb,'Ytick',sample{6},'YTickLabel',[]); drawnow;

alpha = output(1,1,1).w1min/output(1,1,1).w2min;
polmap = intensity.*flimap ./ (1+(alpha-1).*flimap);
