...
ivec = sept15; %[jul31_donor_ran,jul31_uf1_ran,jul31_uf2_ran];
acq_time = 20;
%%%
show_fit=1;
show_images=1;
...
if isempty(c1index)
   ...
   % yspacer = [0,0,0:.01:max(ave(FRET)),0,0]
   % ave_FRET = [FFearly_mix,FFlate_mix,FFsep_mix];
  %  ind = length(ave_FRET);
    x_plot =[];
    shift_time(1) = start_time;
    for i=1:5
    x_plot = [x_plot, shift_time(i):save_time:shift_time(i)+save_time*(50-1)];
    shift_time(i+1) = x_plot(end)+20*save_time;
    end
    x_plot = x_plot/60;
