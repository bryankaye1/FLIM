...
num_int_bins = 0;
combine_exposures_FLIMage =1; %10; %Use for
...
w1step = .01; w1min= 1.6; w1max = 1.6;%1.73            %1.064-2016/2/27  --- %1.73  %1.6 used for extract 8/27 and 9/5 (actual 9/5 is 1.59) %1.5 used for taxol extract; %.8-2 1.05 %w1min must be an integer multiple of w1step.
w2step = .02; w2min =  3.8; w2max = 3.8;%3.802               %3.82/1.49   %3.8 used for 3/7/15 data %3.745 usd for 8/27 E %3.87 used for taxol extract; %3.81 was found for 8/25 b80 and 8/24 extract

fracstep = 0.002; %.005 with w1/w2 set is 10sec per group
prstep = fracstep; prmin=0; prmax = 1;
w02step = fracstep; w02min = 0; w02max = 1;
thr = 0.01;
dataname_cell = {'m4-exposure-2','m4-exposure-10'};%{'m4','m2'};
for dataname_ind = dataname_cell
    for reach = 0:6
        fprintf('\n\n dataname is %s - reach is %s',dataname_ind{1},num2str(reach));
        comment = sprintf('dataname is %s - reach is %s',dataname_ind{1},num2str(reach));
        for expt = 1:exptmax %determined by number of time series
            ...
            for cindex = 1:cyclesmax %determined by number of w2 spots
                ...
                cpath = 'C:\Users\Bryan\Documents\MATLAB\data\2015-3-7\';
                pth_irf = cpath; %file path IRF
                pth_data = cpath; %file path data
                pth_wigs = 'C:\Users\Bryan\Documents\MATLAB\data\2015-2-27\'; %file path wiggles
                pth_ext = 'C:\Users\Bryan\Documents\MATLAB\data\2014-6-13\'; %ignore this
                pth_data_for_shift = cpath;
                irfname = 'irf-538pm';
                data_shift_name = 'donor-only-s1';
                wigsname = 'wigs';
                extname = 'extract-10m'; %IGRNORE THIS
                ...
                while jind < jmax %j is determined by number of pixel groups
                    ...
                    if jind ==1 && combine_exposures_FLIMage > 0
                        for k = 1: combine_exposures_FLIMage
                            dataname2 = dataname; %strcat(dataname,'_c',num2str(k+10)); %make general when cleaning up this code
                            if k==1
                                [combined_FLIMage,jmax,combined_ni] = spc_2_his(tmini,tmaxi,dataname2,pth_data,ngr,tsm,'FLIMage',cpath,'atto565-10um',reach); %atto565 for Pf plots
                            else
