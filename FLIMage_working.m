...
imin = 4010; imax = 4013; split_matin = imax-imin+1;
ind =0;
tic
for i=imin:imax   
    ...
    split_matin = FLIMage_mat(1,1,1).split_matin;
    pstart = 1+(i-1)*(jmax/split_matin);
   % pend = i*(jmax/split_matin);
    
    for j = 1:jmax
    k = pstart+j-1;
    prest(k) = tempf.FLIMage_mat(1,1,j).prest; 
    prestx(:,k) = tempf.FLIMage_mat(1,1,j).prestx;  
...
%     FLIMage_mat(:,:,:).prest = output(:,:,:).prest;
%     FLIMage_mat(:,:,:).prestx = output(:,:,:).prestx;
%     FLIMage_mat(:,:,:).w02est = output(:,:,:).w02est;
%     FLIMage_mat(:,:,:).w02estx = output(:,:,:).w02estx;
%     FLIMage_mat(:,:,:).ni = output(:,:,:).ni;
%     FLIMage_mat(1,1,1).w1 = w1Best;
%     FLIMage_mat(1,1,1).w2 = w2Best;
