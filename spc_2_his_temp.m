function [data,jmax, nphi,varargout] = spc_2_his_temp(tmini,tmaxi,file_name,pth_sdt,ngr,tsm,varargin)
%sort pixels by intensity, equal pixels per group
%Vargin(1) = 'imout' for only getting an image out and a single 



%varagout{2} = image out if varargin1 or varargin3 is 
%varargout{1} = sinti/mean(sinti);
...
%% return FLIM vectors
    ...
    %and relative intensity (sinti)
        ...
        for gr = 1:ngr
            ...
            sinti(gr) = pmtgroup/sint; %This devision reutrns effective int_map_sort (or epsilon)
        end
        varargout{1} = sinti;
    end
    data=dataout';
