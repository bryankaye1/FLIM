...
function [ymat,varmat,imat,polmat,monmat,mask_distance] = FF_vs_dist3(ivec,...
...
for i = ivec
    ...
    ti = [];%ti = strrep([output(1,1,1).pth_data(end-10:end),output(1,1,1).dataname],'_',' ');
    ...
    switch isfield(seg_results,'div_groups')
        case 0
            ...
            if donor_offset
                ...
                eNmon = intensity.*(pf-(y-0)) ./ (pf*(1+(al-1).*(y-0)));
            end
            ep = mean(eNmon(end-9:end))/tub_con;
            pol_norm = eNpol / ep;
        ...
        case 1
            ...
            for dg=1:dgmax
                ...
                eNmon = intensity_dg.*(pf-(y_dg-y0)) ./ (pf*(1+(al-1).*(y_dg-y0)));
                %mon = mon/max(mon);
                
                [eNpol,polstd] = calc_pol(al,pf,y_dg-y0,stdpr_dg,intensity_dg);
                ep = mean(eNmon(end-6:end-1))/tub_con;
                pol_norm = eNpol/ ep;
                polstd_norm = polstd / ep;
                
                fprintf('ep is %3.1f pol is %3.0f int is %3.0f fret is %3.2f\n',...
                    ep,max(pol_norm),max(intensity_dg),max(y_dg));
    ...
    if length(ivec)==1
        ...
        if  average_spindles
            ...
            for dg = [4,3,1,2]
                ...
                Npolave_norm{dg} = polave;
            end
            figure;
            for dg = 1:dgmax
                plot(mask_distance_ave,Npolave_norm{dg},'Color',quad_colors{dg},...
                    'MarkerSize',msize_dot); hold on;
                xlabel('distance (microns)');
                ylabel('Polymer Concentration (au)');
                axis(ax_lim_pol);
        ...
        [hd_comb(1),pd_comb(1)] = ttest([hwhmat{1},hwhmat{4}],...
            [hwhmat{2},hwhmat{3}])
...
if average_spindles && ~isfield(seg_results,'div_groups') && length(ivec)>1
    if ~(ivec(1)==30735)
        y_ave = mean(ymat,1);
        i_ave = mean(imat,1);
        Nmon_ave = mean(monmat,1);
        std_ave = sqrt(mean(varmat,1)/(ind));
        mask_distance_ave = mean(mask_distancemat,1);
        donor = 0;
        
        Npolave_norm = mean(polmat,1);
        Npolave_std_norm = std(polmat,1);
        %polave_norm = polave/max(polave);
        %polave_std_norm = polave_std / max(polave);
        
        figure(ind+1); clf; hold on;
        yyaxis left;
        errorbar(mask_distance_ave,Npolave_norm,Npolave_std_norm/sqrt(11));
        ylabel('MT concentration (uM)');
        axis([-5 15 0 ax_pol_hi]);
        yyaxis right;
        plot(mask_distance_ave,Nmon_ave);
        ...
        Npolave_norm = polave/max(polave);
        Npolave_std_norm = polave_std / max(polave);
        ...
        FFdist_plots(ind+1,mask_distance,maxlen,Npolave_norm,Npolave_std_norm,...
