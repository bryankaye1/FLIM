...
july31_UF_endpoints = [15057:2:15063,15058:2:15064];
...
ivec = july31_UF_endpoints;%[22606:22655,22707:22755,22756: 22805]; %[jul31_donor_ran,jul31_uf1_ran,jul31_uf2_ran];
acq_time = 100;
%%%
show_fit=1;
show_images=1;
...
%     end
    ...
    pol(ind) = sum( x_int.*y./( pf *( 1 + (al-1).*y) ));
    ave_FRET(ind) = sum(y.*x_int./(1-y+al*y))/sum(x_int./(1-y+al*y)); %fraction of guys engaged in FRET
    
    yw = (x_int./(1-y+al*y))/sum(x_int./(1-y+al*y));
    ywsig = (sqrt(x_int)./(1-y+al*y))/sum(x_int./(1-y+al*y));
...
for j = 1:ind
    ...
    yyaxis left;
    plot(x_plot(1:j),ave_xint_mat(1:j)','.','MarkerSize',10);%/(pol(1)/sumxmat(1))
    %plot(x_plot(1:j),pol(1:j),'g.','MarkerSize',10);
    ylim([0,1.1*max(ave_xint_mat)]);
    ylabel('Total Photons');
    
    yyaxis right;
    %plot(x_plot(1:j),ave_FRET(1:j),'.','MarkerSize',10);
    
    errorbar(x_plot(1:j),ave_FRET(1:j),ave_FRETerr(1:j),'.','MarkerSize',10);
    ylabel('FRET fraction');
...
movie2avi(F,'C:\Users\Bryan\Documents\MATLAB\movies\7_31_coverasters','compression','none');
toc